...
const CarouselGenerator: React.FC = () => {
  ...
  const EXTREME_STEPS = extremeAnalysis
    ...
    : ['Modo', 'Visão'];
  const TWEET_STEPS = ['Modo', 'Tweet Config', 'Tema', ...(tweetConfig.photoMode === 'web' ? ['Pesquisa'] : []), ...(tweetConfig.photoMode === 'web' && !tweetConfig.autoSelectPhotos && showFotosWebStep ? ['Fotos'] : []), 'Roteiro Tweet'];
