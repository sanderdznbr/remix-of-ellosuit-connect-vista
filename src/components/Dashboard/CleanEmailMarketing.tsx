...
const CleanEmailMarketing: React.FC = () => {
  ...
  const renderConnectionTab = () => (
    <div className="space-y-6">
      ...
      {/* Connection Options */}
      <div>
        <h3 className="text-lg font-semibold text-gray-900 mb-4">Conecte seu email para enviar campanhas</h3>
        
        {/* Gmail Option - Single card, full width */}
        <Card className={`cursor-pointer transition-all hover:shadow-md ${isConnected ? 'border-[#3000E3] ring-2 ring-[#3000E3]/20' : 'border-gray-200'}`}>
          <CardContent className="p-6">
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1">
                <h4 className="font-semibold text-gray-900 mb-1">Gmail / Google Workspace</h4>
                <p className="text-sm text-gray-500 mb-3">
                  Conecte seu Gmail ou conta Google Workspace para enviar emails do seu próprio endereço.
                </p>
                <div className="flex items-center gap-2 text-xs text-gray-400 mb-4">
                  <Badge variant="outline" className="text-xs">500-2000 emails/dia</Badge>
                  <Badge variant="outline" className="text-xs">Seu próprio email</Badge>
                  <Badge variant="outline" className="text-xs">Respostas na sua caixa</Badge>
                </div>
                
                {isConnected ? (
                  <div className="flex items-center gap-2 text-green-600">
                    <CheckCircle2 className="h-4 w-4" />
                    <span className="text-sm font-medium">Conectado como {emailAccount?.provider_email}</span>
                  </div>
                ) : (
                  <Button 
                    onClick={connectGmail}
                    disabled={loading}
                    className="bg-[#3000E3] hover:bg-[#2500B3]"
                  >
                    {loading ? (
                      <>
