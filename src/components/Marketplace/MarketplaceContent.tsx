...
import { ShoppingBag, Star, Sparkles, Check, ArrowLeft, Filter, Search, Crown, Zap } from 'lucide-react';
...
const MarketplaceContent: React.FC = () => {
  ...
  const handlePurchase = async (style: MarketplaceStyle) => {
    ...
    if (purchasedIds.has(style.id)) {
      toast.info('Você já possui este estilo!');
      return;
    }
    if (creditBalance < style.price_credits) {
      toast.error('Créditos insuficientes. Compre mais créditos para adquirir este estilo.');
      navigate('/precos');
      return;
    }
  ...
  return (
    <div className="flex-1 h-screen overflow-y-auto" style={{ backgroundColor: '#0a0a0f' }}>
      {/* Header */}
      <div className="sticky top-0 z-20 backdrop-blur-xl border-b border-white/[0.06]" style={{ backgroundColor: 'rgba(10,10,15,0.85)' }}>
        <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-4">
            <button onClick={() => navigate('/')} className="p-2 rounded-lg hover:bg-white/[0.06] text-white/50 hover:text-white transition-colors cursor-pointer">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-white/40">Estilos premium para seus carrosséis</p>
              </div>
            </div>
          </div>
          ...
          {user && (
            <div className="flex items-center gap-3">
              <div className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-white/[0.06] text-sm">
                <Zap className="w-4 h-4 text-yellow-400" />
                <span className="text-white/70">{Math.floor(creditBalance)} créditos</span>
              </div>
            </div>
          )}
        </div>
      ...
      <div className="max-w-7xl mx-auto px-6 py-12">
        <div className="text-center mb-12">
          <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-purple-500/10 border border-purple-500/20 text-purple-400 text-sm mb-4">
            ...
            Estilos exclusivos por IA
          </div>
          <h2 className="text-3xl md:text-4xl font-bold text-white mb-3">
            Transforme seus posts com estilos{' '}
            <span className="bg-gradient-to-r from-purple-400 to-pink-400 bg-clip-text text-transparent">únicos</span>
          </h2>
          <p className="text-white/40 max-w-lg mx-auto">
            Estilos criados por designers profissionais para elevar seus carrosséis a outro nível.
          </p>
        </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
                  {featured.map(style => (
                    <StyleCard
                      ...
                      purchasing={purchasing === style.id}
                      onPurchase={() => handlePurchase(style)}
      ...
      {selectedStyle && (
        <StyleDetailModal
          style={selectedStyle}
          owned={purchasedIds.has(selectedStyle.id)}
          purchasing={purchasing === selectedStyle.id}
          onPurchase={() => handlePurchase(selectedStyle)}
...
const StyleCard: React.FC<StyleCardProps> = ({ style, owned, purchasing, onPurchase, onPreview, featured }) => {
    ...
    >
      ...
      <div className="aspect-[4/5] relative overflow-hidden bg-white/[0.03]">
        ...
        <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
          <span className="text-white text-sm font-medium px-4 py-2 rounded-lg bg-white/10 backdrop-blur-sm">
            Ver detalhes
          </span>
        </div>
        {featured && (
      ...
      <div className="p-4">
        ...
        {style.description && (
          <p className="text-xs text-white/30 mt-1 line-clamp-2">{style.description}</p>
        )}
...
const StyleDetailModal: React.FC<StyleDetailModalProps> = ({ style, owned, purchasing, onPurchase, onClose }) => {
                ...
                {style.preview_images.map((img, i) => (
                  <button
                    ...
                    className={`w-14 h-14 rounded-lg overflow-hidden border-2 transition-colors cursor-pointer ${
                      i === activeImage ? 'border-purple-500' : 'border-transparent opacity-50 hover:opacity-80'
                    }`}
                  >
              ...
              {!owned ? (
                <>
                  <div className="flex items-baseline gap-2">
                    ...
                    {style.price_brl > 0 && (
                      <span className="text-xs text-white/20 ml-2">(~R$ {style.price_brl.toFixed(2)})</span>
                    )}
                  </div>
                  <button
                    onClick={onPurchase}
                    disabled={purchasing}
                    className="w-full py-3 rounded-xl bg-gradient-to-r from-purple-600 to-pink-600 text-white font-semibold text-sm hover:from-purple-500 hover:to-pink-500 transition-all disabled:opacity-50 cursor-pointer flex items-center justify-center gap-2"
                  >
                    {purchasing ? (
