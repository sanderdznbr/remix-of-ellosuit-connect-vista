...
- NÃO gere uma interface genérica ou inventada, mas também NÃO copie literalmente textos de branding da screenshot.`;
    ...
    if (isVisualCloneMode) {
      ...
      if (validFaceRefs.length > 0) {
        ...
        messageContent.push({ type: 'text', text: `⚠️ AVISO CRÍTICO ANTES DAS REFERÊNCIAS: As próximas ${validStyleRefs.length} imagens são APENAS referências de ESTILO VISUAL. Os textos, títulos, nomes, placares, resultados, marcas e QUALQUER conteúdo textual visível nessas imagens são de OUTROS POSTS e DEVEM SER 100% IGNORADOS. As FOTOGRAFIAS, CENAS e PESSOAS nessas imagens também são de OUTROS PROJETOS — NÃO reproduza, copie ou recrie essas fotos/cenas. Extraia APENAS: paleta de cores, tipografia/fonte (família, peso, estilo), layout, elementos gráficos decorativos e composição visual. Gere fotos e cenas NOVAS e RELEVANTES ao tema do post.` });
        // Send all style refs
