...
// Tag configurations
const TAG_CONFIGS: Record<string, { label: string; color: string; icon: React.ElementType }> = {
  cliente: { label: 'Cliente', color: 'bg-blue-100 text-blue-700 border-blue-200', icon: Users },
  fornecedor: { label: 'Fornecedor', color: 'bg-amber-100 text-amber-700 border-amber-200', icon: Building2 },
  prospecto: { label: 'Prospecto', color: 'bg-purple-100 text-purple-700 border-purple-200', icon: Target },
  colaborador: { label: 'Colaborador', color: 'bg-green-100 text-green-700 border-green-200', icon: Briefcase },
};
...
const STATUS_CONFIGS: Record<string, { label: string; color: string }> = {
  ...
  prospect: { label: 'Lead', color: 'bg-orange-100 text-orange-700' },
};
...
const UnifiedDatabase: React.FC = () => {
  ...
  // Load all clients
  const { clients, loading, createClient, updateClient, deleteClient } = useClients('all');
      ...
      <Card className="border-0 shadow-sm">
        <CardContent className="p-4">
          <div className="flex flex-col lg:flex-row gap-4">
            ...
            {/* Clear Filters */}
            {(selectedTags.length > 0 || selectedStatus.length > 0 || searchTerm) && (
            ...
            <TableHeader>
              <TableRow className="bg-gray-50/80">
                <TableHead className="w-12">
                  <Checkbox 
                    checked={selectedRows.length === filteredClients.length && filteredClients.length > 0}
                  ...
                  return (
                    <TableRow 
                      ...
                      className={cn(
                        ...
                        selectedRows.includes(client.id) && "bg-blue-50/50"
                      )}
                    >
                      <TableCell>
                        <Checkbox 
