...
import FontChangePicker from './editor/FontChangePicker';
...
const FONT_OPTIONS = [
  ...
  // Animated-friendly display fonts
  { label: 'Dela Gothic One', value: "'Dela Gothic One', 'Impact', sans-serif", google: 'Dela+Gothic+One' },
  { label: 'Righteous', value: "'Righteous', 'Impact', sans-serif", google: 'Righteous' },
  { label: 'Teko', value: "'Teko', 'Impact', sans-serif", google: 'Teko:wght@400;500;600;700' },
  { label: 'Unbounded', value: "'Unbounded', 'Helvetica Neue', sans-serif", google: 'Unbounded:wght@400;500;600;700;800;900' },
  { label: 'Lexend', value: "'Lexend', 'Helvetica Neue', sans-serif", google: 'Lexend:wght@400;500;600;700;800;900' },
  { label: 'Plus Jakarta Sans', value: "'Plus Jakarta Sans', 'Helvetica Neue', sans-serif", google: 'Plus+Jakarta+Sans:wght@400;500;600;700;800' },
  { label: 'Rubik', value: "'Rubik', 'Helvetica Neue', sans-serif", google: 'Rubik:wght@400;500;600;700;800;900' },
  { label: 'Bungee', value: "'Bungee', 'Impact', sans-serif", google: 'Bungee' },
  { label: 'Secular One', value: "'Secular One', 'Impact', sans-serif", google: 'Secular+One' },
  { label: 'Black Ops One', value: "'Black Ops One', 'Impact', sans-serif", google: 'Black+Ops+One' },
];
...
const CarouselGenerator: React.FC = () => {
  ...
  const TWEET_STEPS = ['Modo', 'Tweet Config', 'Tema', ...(showPesquisaStep ? ['Pesquisa'] : []), ...(showFotosWebStep ? ['Fotos'] : []), ...(showTweetProductStep ? ['Produto'] : []), 'Roteiro Tweet', ...adminModelStep];
  const TWEET2_STEPS = ['Modo', 'tweet2', 'Tema', ...(showPesquisaStep ? ['Pesquisa'] : []), ...(tweet2Config.photoMode === 'web' && showFotosWebStep ? ['Fotos'] : []), 'Roteiro Tweet2', ...(tweet2Config.photoMode === 'manual' ? ['Fotos Tweet2'] : []), ...adminModelStep];
  ...
  // Pre-fill wizard from /criar (chat) redirect: ?topic=&styleId=&format=&autostart=1
  useEffect(() => {
    ...
    if (qAutostart) setSkipWebSearch(true);
  ...
  const buildImagePrompt = (basePrompt: string, cardIndex?: number): string => {
    ...
    // === ANTI-AI AESTHETIC (MANDATORY) ===
    parts.push(`ANTI-AI BACKGROUND RULES (CRITICAL — NEVER VIOLATE):
ABSOLUTELY FORBIDDEN backgrounds: glowing particles, floating orbs, abstract network/connection lines, hexagonal grids, digital matrix effects, generic blue/purple tech gradients, bokeh circles, lens flares, neural network visuals, geometric wireframes, constellation patterns, holographic effects, generic "tech" or "futuristic" backgrounds.
These look cheap and AI-generated. NEVER use them.

INSTEAD, use ONE of these approaches for backgrounds:
1. SOLID COLOR or subtle gradient (clean, editorial, Apple-style)
2. REAL ENVIRONMENT — concrete wall, wood desk, marble surface, studio backdrop, urban street, coffee shop, office, nature
3. HANDS INTERACTING — real human hands holding the device, typing, swiping, pointing at screen
4. LIFESTYLE CONTEXT — person using the product in a real setting (desk, couch, meeting room, outdoor)
5. MACRO TEXTURES — fabric, paper grain, brushed metal, leather, stone
6. PHOTOGRAPHY-BASED — shallow depth of field with real objects, NOT digital effects
7. MINIMALIST — clean white/black/colored background with subtle shadows (product photography style)

The image must look like it was shot by a professional photographer or designed by a top-tier graphic designer — NOT generated by AI. Prioritize REALISM, TANGIBILITY and EDITORIAL quality.`);

    // === ANTI-SWIPE INDICATORS (for all images) ===
    parts.push('PROIBIDO: NUNCA inclua setas apontando para o lado, ícones de "swipe", textos como "Arraste para o lado", "Deslize →", "Swipe", contadores de slides (1/5, 2/5), ou qualquer indicador de navegação lateral dentro da imagem gerada.');
  ...
  // ===== CHANGE FONT ON CARD =====
  const changeCardFont = useCallback(async (fontName: string, fontPreviewUrl?: string) => {
    if (!carouselData || changingFont) return;
    ...
    try {
      const textContent = {
        ...
        body: card.body || '',
        bodyTop: card.bodyTop || '',
        bodyBottom: card.bodyBottom || '',
      };

      const { data, error } = await supabase.functions.invoke('change-card-font', {
        body: {
          imageUrl: card.imageUrlRaw || card.imageUrl,
          textContent,
          fontReference: fontName,
          fontPreviewUrl: fontPreviewUrl || undefined,
          accentColor,
        },
      });
      ...
      if (!data?.success || !data?.imageUrl) {
        throw new Error(data?.error || 'Falha ao mudar fonte');
      }
      ...
      newCards[cardIdx] = {
        ...
        // Keep raw image for logo repositioning
        imageUrlRaw: card.imageUrlRaw || card.imageUrl,
      };
      ...
      if (logoUrl) {
        try {
          ...
          // Store the font-changed image as raw (without logo) for future repositioning
          newCards[cardIdx] = {
            ...newCards[cardIdx],
          ...
          console.warn('[CHANGE_FONT] Logo re-apply failed:', logoErr);
        }
      }

      setCarouselData(prev => prev ? { ...prev, cards: newCards } : prev);
      toast({ title: 'Fonte alterada com sucesso!' });
    } catch (err: any) {
      console.error('[CHANGE_FONT] Error:', err);
      const msg = err?.message || 'Erro ao mudar fonte';
    ...
    try {
      ...
      if (tweetConfig.photoMode === 'web') {
        ...
        for (let i = 0; i < cards.length; i++) {
          ...
          const productRefs = productImages.length > 0 ? productImages.map(p => p.url || p.thumb).filter(Boolean) : [];
  ...
  const generateSinglePost = async () => {
    ...
    try {
      ...
      promptParts.push('PROIBIDO (POST ÚNICO/ESTÁTICO): Este é um post ÚNICO, NÃO é um carrossel. NUNCA inclua setas apontando para o lado, ícones de "swipe", textos como "Arraste para o lado", "Deslize", "Swipe", indicadores de navegação lateral ou qualquer elemento que sugira múltiplos slides. NÃO inclua contadores de slides (1/5, 2/5) nem indicadores de posição. A imagem deve ser AUTÔNOMA e COMPLETA.');
    ...
    try {
      ...
      for (let i = 0; i < cardCount; i++) {
        ...
        const payload: Record<string, any> = {
          ...
          backgroundStyle: animatedBgStyle,
    ...
    <div className="h-screen flex flex-col overflow-y-auto" style={{ backgroundColor: '#0A0A0A' }}>
        ...
        {showWelcome && user && (
          ...
          >
            <DashboardLayout
              ...
              onResumeJob={(jobId: string) => {
                // Re-enter generating animation and subscribe to this cloud job
                setCloudJobId(jobId);
                setGenerating(true);
                setShowWelcome(false);
                setImageGenProgress('');
      ...
      <div className={carouselData && editingCard === null ? '' : 'flex-1 flex flex-col'} style={carouselData && editingCard === null ? { flex: 1, display: 'flex', flexDirection: 'column' } : undefined}>
          ...
          <div className="flex-1 flex flex-col w-full relative overflow-hidden" style={{ backgroundColor: '#0A0A0A', paddingTop: 'env(safe-area-inset-top)' }}>
            ...
            <div className="relative z-20 flex items-center justify-between px-4 sm:px-6 pt-3 pb-2">
              ...
              {isMobileView ? (
                ...
                /* Desktop/Tablet: same compact progress bar as mobile */
                <div className="flex items-center gap-2 flex-1 max-w-[320px]">
                  <div className="flex-1 flex flex-col gap-1">
                    <div className="flex items-center justify-between">
                      <span className="text-[11px] font-medium" style={{ color: modeTheme.hex }}>
                        {(() => {
                          const shortNames: Record<string, string> = {
                            'Modo': 'Modo', 'Estilo': 'Estilo', 'Tema': 'Tema', 'Formato': 'Qtd',
                            'Personalização': 'Brand', 'Velocidade': 'Velocidade', 'Roteiro': 'Roteiro',
                      ...
                      <span className="text-[10px] text-white/20">{currentWizardDisplayStep}/{visibleWizardSteps.length}</span>
                    </div>
                    <div className="w-full h-[3px] rounded-full bg-white/[0.06] overflow-hidden">
                      <div
                        className="h-full rounded-full transition-all duration-300"
            ...
            <div className="flex-1 flex flex-row relative z-10 w-full overflow-hidden">
              ...
              >
                <div className="min-h-full flex items-center justify-center py-[clamp(1rem,4vh,3rem)]">
                  <div className={`w-full space-y-6 flex-shrink-0 ${currentStepName === 'Estilo' ? '' : 'max-w-[560px]'}`}>
                        ...
                        {generatingRoteiro ? (
                          ...
                          <>
                            ...
                            <button
                              ...
                              <Wand2 className="h-4 w-4" /> {tweetConfig.tweetTexts.some(t => t.trim()) ? 'Regenerar roteiro' : 'Gerar roteiro com IA'}
                            </button>
                          </>
                        )}
                      </div>
        ...
        {carouselData && editingCard === null && (() => {
          ...
          >
                      ...
                      <>
                        ...
                        {!isGuest && (
                          <button
                            ...
                            <Undo2 className="h-4 w-4 text-yellow-400" /> Voltar edição {correctionUndoStack.length > 0 && <span className="ml-auto text-[10px] text-yellow-400/60">({correctionUndoStack.length})</span>}
                          </button>
                        )}
              ...
              <div className="w-full flex flex-col md:flex-row md:items-center md:justify-between md:gap-5 items-center relative md:min-h-[calc(100vh-6rem)] md:px-5" style={{ paddingBottom: isMobileView ? 'calc(5.5rem + env(safe-area-inset-bottom, 0px))' : '0' }}>
                ...
                >
                  ...
                  {(() => {
                    ...
                    const isDesk = vw >= 768;
                    const maxByH = isDesk ? Math.min(vh - 180, 720) : 420;
                    const maxByW = isDesk ? Math.min(vw - 720, 520) : vw - 80;
                    const baseW = isDesk ? Math.max(440, Math.min(maxByW, maxByH * (cardW / cardH))) : Math.max(320, Math.min(maxByW, maxByH * (cardW / cardH)));
                  ...
                  >
                    ...
                    >
                      ...
                      <div className="px-3 py-3 max-h-[55vh] overflow-y-auto space-y-0.5">
                        ...
                        {!isGuest && (
                          <button onClick={() => { setShowMobileToolsSheet(false); setRegenDialogCard(activeCardIndex); }}
                            ...
                            <div className="w-11 h-11 rounded-2xl flex-shrink-0 flex items-center justify-center overflow-hidden" style={{ background: 'linear-gradient(135deg, rgba(59,130,246,0.12), rgba(59,130,246,0.04))' }}>
                              {regeneratingCard === activeCardIndex ? <Loader2 className="h-5 w-5 text-sky-400 animate-spin" /> : <Wand2 className="h-5 w-5 text-sky-400" />}
                            </div>
                            <div className="flex-1 min-w-0">
                        ...
                        {/* Undo */}
                        {!isGuest && (
                          <button onClick={() => { setShowMobileToolsSheet(false); handleUndoEdit(); }}
                            disabled={correctionUndoStack.length === 0}
                            className="w-full flex items-center gap-3.5 px-3 py-3 rounded-2xl hover:bg-white/[0.04] active:bg-white/[0.06] transition-all text-left group disabled:opacity-40">
                            ...
                            <div className="flex-1 min-w-0">
                              <span className="text-sm font-semibold text-white/90 block group-hover:text-white transition-colors">Voltar edição</span>
                              <span className="text-[11px] text-white/25 leading-tight">Desfazer última alteração {correctionUndoStack.length > 0 ? `(${correctionUndoStack.length})` : ''}</span>
                            </div>
                          </button>
                        )}
                        ...
                        {/* Redo */}
                        {!isGuest && (
                          <button onClick={() => { setShowMobileToolsSheet(false); handleRedoEdit(); }}
                            disabled={correctionRedoStack.length === 0}
                            className="w-full flex items-center gap-3.5 px-3 py-3 rounded-2xl hover:bg-white/[0.04] active:bg-white/[0.06] transition-all text-left group disabled:opacity-40">
            ...
            {resultViewMode === 'basic' && isMobileView && !isGuest && !showMobileToolsSheet && !showCardActionSheet && (
      ...
      <AnimatePresence>
        {pendingRecreateConfig && (() => {
          const activeFaces = facePersons.filter(p => p.photos.length > 0);
          const brandRefs = referenceImages.filter(r => r.category === 'brand');
          const hasAnyAsset = !!logoUrl || activeFaces.length > 0 || brandRefs.length > 0 || styleScreenshots.length > 0 || productImages.length > 0;
          return (
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            ...
            <motion.div
              ...
              onClick={(e) => e.stopPropagation()}
              className="relative w-full max-w-[380px] rounded-3xl overflow-hidden overflow-y-auto max-h-[85dvh]"
              style={{ backgroundColor: 'rgba(6,6,6,0.92)', backdropFilter: 'blur(40px)', WebkitBackdropFilter: 'blur(40px)', border: '1px solid rgba(255,255,255,0.06)', boxShadow: '0 32px 64px rgba(0,0,0,0.6)' }}>
              
              {/* Header */}
              <div className="flex items-center justify-between px-5 pt-5 pb-3">
                <div className="flex items-center gap-3">
                  <div className="w-9 h-9 rounded-xl flex items-center justify-center" style={{ background: 'linear-gradient(135deg, hsl(var(--primary)/0.2), hsl(var(--primary)/0.05))' }}>
                    <Sparkles className="h-4 w-4" style={{ color: 'hsl(var(--primary))' }} />
                  </div>
                  <h3 className="text-base font-semibold text-white tracking-tight">Confirmar geração</h3>
                </div>
                <button onClick={() => setPendingRecreateConfig(null)} className="w-8 h-8 rounded-full flex items-center justify-center text-white/40 hover:text-white/70 hover:bg-white/5 transition-all cursor-pointer">
                  <X className="h-4 w-4" />
                </button>
              </div>
              ...
              <div className="px-5 pb-5 space-y-4">
                ...
                {hasAnyAsset && (
                  <div className="space-y-3">
                    ...
                    {activeFaces.length > 0 && (
                      <div className="rounded-2xl p-3.5" style={{ backgroundColor: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.05)' }}>
                        <p className="text-[10px] font-semibold text-white/35 uppercase tracking-[0.08em] mb-2.5">Rosto</p>
                    ...
                    {logoUrl && (
                      <div className="rounded-2xl p-3.5" style={{ backgroundColor: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.05)' }}>
                        <p className="text-[10px] font-semibold text-white/35 uppercase tracking-[0.08em] mb-2.5">Marca</p>
                        <div className="flex items-center gap-3">
                          <div className="w-11 h-11 rounded-xl overflow-hidden ring-1 ring-white/10 flex-shrink-0" style={{ backgroundColor: 'rgba(255,255,255,0.05)' }}>
                            <img src={logoUrl} alt="Logo" className="w-full h-full object-contain" />
                          </div>
                          <div className="flex-1 min-w-0">
                            <p className="text-xs text-white/60 truncate">{brandName || 'Logo enviada'}</p>
                            {logoBrandColors.length > 0 && (
                              <div className="flex gap-1 mt-1.5">
                                {logoBrandColors.map((c, i) => (
                                  <div key={i} className="w-4 h-4 rounded-md ring-1 ring-white/10" style={{ backgroundColor: c }} />
                                ))}
                              </div>
                            )}
                          </div>
                    ...
                    {/* Brand references + Screenshots + Product — compact row */}
                    {(brandRefs.length > 0 || styleScreenshots.length > 0 || productImages.length > 0) && (
                      <div className="rounded-2xl p-3.5" style={{ backgroundColor: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.05)' }}>
                        <p className="text-[10px] font-semibold text-white/35 uppercase tracking-[0.08em] mb-2.5">Referências</p>
                        <div className="flex gap-2 flex-wrap">
                          {brandRefs.map((ref, i) => (
                            <div key={`brand-${i}`} className="w-11 h-11 rounded-xl overflow-hidden ring-1 ring-white/10">
                              <img src={ref.thumb} alt={ref.label} className="w-full h-full object-cover" />
                            </div>
                          ))}
                          {styleScreenshots.map((s, i) => (
                            <div key={`ss-${i}`} className="w-11 h-11 rounded-xl overflow-hidden ring-1 ring-white/10">
                              <img src={s.thumb || s.url} alt="" className="w-full h-full object-cover" />
                            </div>
                          ))}
                          {productImages.map((p, i) => (
                            <div key={`prod-${i}`} className="w-11 h-11 rounded-xl overflow-hidden ring-1 ring-white/10">
                ...
                {logoBrandColors.length > 0 && (
                  <div className="flex items-center justify-between py-3 px-4 rounded-2xl" style={{ backgroundColor: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.05)' }}>
                    <div className="flex items-center gap-2.5">
                      <Palette className="h-3.5 w-3.5 text-white/35" />
                      <span className="text-xs text-white/60">Usar cores da marca</span>
                    </div>
                    <button
                ...
                <div className="space-y-2">
                  <p className="text-[10px] font-semibold text-white/35 uppercase tracking-[0.08em]">Ideia visual (opcional)</p>
                  <textarea
                ...
                <div className="flex gap-2.5 pt-1">
                  <button
                    ...
                    className="flex-1 py-3 rounded-2xl text-sm font-semibold text-white transition-all hover:opacity-90 cursor-pointer flex items-center justify-center gap-2"
                    style={{ background: 'linear-gradient(135deg, hsl(var(--primary)) 0%, hsl(var(--primary)/0.7) 100%)', boxShadow: '0 4px 20px hsl(var(--primary)/0.25)' }}>
                    <Sparkles className="h-4 w-4" />
                    Gerar agora
                  </button>
                  <button
                    onClick={() => setPendingRecreateConfig(null)}
                    className="px-5 py-3 rounded-2xl text-xs font-medium text-white/40 hover:text-white/60 transition-colors cursor-pointer"
