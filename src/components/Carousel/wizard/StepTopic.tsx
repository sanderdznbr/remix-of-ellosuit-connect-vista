...
interface Props {
  ...
  fromTrendData?: { topic: string; format: string; cardText: string; cardTexts?: { title: string; subtitle: string }[]; caption: string } | null;
}
        ...
        {isCarousel && Array.isArray(fromTrendData.cardTexts) && fromTrendData.cardTexts.length > 0 ? (
          <div className="rounded-xl border border-white/[0.06] p-4" style={{ backgroundColor: 'rgba(139,92,246,0.03)' }}>
            ...
            <div className="space-y-2">
              {fromTrendData.cardTexts.map((item, index) => {
                const title = typeof item === 'string' ? item : item?.title || '';
                const subtitle = typeof item === 'string' ? '' : item?.subtitle || '';
                return (
                  <div key={`${index}-${title}`} className="flex items-start gap-2 rounded-lg border border-white/[0.05] px-3 py-2" style={{ backgroundColor: 'rgba(255,255,255,0.02)' }}>
                    ...
                    <div>
                      ...
                      {subtitle && <p className="text-xs text-white/40 leading-relaxed">{subtitle}</p>}
                    </div>
                  </div>
                );
              })}
