...
import { 
  Plus, Play, Pause, Settings, Trash2, GitBranch, 
  BarChart3, Clock, MessageSquare, Users, Eye,
...
const ChatbotManagement: React.FC = () => {
  ...
  const handleCreateFlow = async () => {
    ...
    toast({ title: 'Sucesso!', description: 'Chatbot criado. Clique em "Editar" para construir o fluxo.' });
  ...
  const handleToggleFlow = async (flowId: string, currentActive: boolean) => {
    ...
    if (error) {
      ...
      setFlows(prev => prev.map(f => 
        f.id === flowId ? { ...f, is_active: !currentActive } : f
      ));
      toast({ 
  ...
  const getTriggerLabel = (config: any) => {
    ...
    switch (config.type) {
      case 'keyword': return `Palavra-chave: ${config.value || '...'}`;
      case 'whatsapp_channel': return 'Canal WhatsApp';
      case 'start': return 'Início da conversa';
      case 'schedule': return 'Horário específico';
          ...
          <div className="space-y-4 py-4">
            ...
            {newFlow.triggerType === 'keyword' && (
              <div className="space-y-2">
                ...
                <Input
                  placeholder="Ex: preço, valor, orçamento"
                  value={newFlow.triggerValue}
